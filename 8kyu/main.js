...
select.addEventListener('change', event => {
    ...
    doggoInfo.assingFunFact()
})

const img = document.querySelector('.dog-img')

function setDoggoImg(url) {
    fetch(url)
        .then(res => res.json())
        .then(data => {
...
const doggoInfo = {
    ...
    assignMF() {
        ...
        let flipMF = (Math.floor(Math.random() * 2) == 0)
        if(flipMF) {
            this.MF = 'Female'
        ...
        document.querySelector('.MF').innerHTML = `S : ${this.MF}`           
    },

    assignName(arr) {
        this.rname = arr[Math.floor(Math.random() * arr.length)]
        document.querySelector('.dog-name').innerHTML = this.rname
    },
    ...
    assignLikes() {
        this.dogLike = this.likesList[Math.floor(Math.random() * this.likesList.length)]
        document.querySelector('.likes').innerHTML = `Like: ${this.dogLike}`
    },

    assignDislikes() {
        this.dogDislike = this.dislikesList[Math.floor(Math.random() * this.dislikesList.length)]
        document.querySelector('.dislikes').innerHTML = `Dislike: ${this.dogDislike}`
    },

    assingFunFact() {
        this.dogFunFact = this.factList[Math.floor(Math.random() * this.factList.length)]
        document.querySelector('.fun-fact').innerHTML = `Additional Info: ${this.dogFunFact} `
    }
}